...
import { ExternalLink, Heart, Search, Globe, Server, ShoppingCart, ArrowUpDown, Grid, List } from 'lucide-react';
...
const Domains = () => {
    ...
    return domainsData
      ...
      .sort((a, b) => {
        ...
        if (sortBy === 'name') {
          ...
          comparison = a.registrationPrice - b.registrationPrice;
  ...
  const getHostingBadge = (hosting: string) => {
    const colors: Record<string, string> = {
      'UD': 'bg-purple-500',
      'Spaceship': 'bg-blue-500',
      'Godaddy': 'bg-green-500',
      'Hostinger': 'bg-orange-500',
      'Namebright': 'bg-pink-500'
    };
  ...
  const getBusinessTypeBadge = (type: string) => {
    if (!type) return null;
    return (
      <Badge variant={type === 'Business' ? 'default' : 'secondary'} className="text-xs">
        {type}
      </Badge>
    );
  };
        ...
        <section className="py-12 md:py-20 px-4 sm:px-6 lg:px-8 bg-gradient-to-b from-primary/10 to-background">
          <div className="container mx-auto max-w-7xl text-center">
            <h1 className="text-3xl md:text-5xl font-bold mb-4">
              Premium <span className="gradient-text">Domain Marketplace</span>
            </h1>
            <p className="text-lg md:text-xl text-muted-foreground max-w-3xl mx-auto mb-8">
              Browse 170+ premium domains for your startup, SaaS, fintech, or e-commerce business.
              Search across 16+ platforms or explore our curated collection.
            </p>
                    ...
                    <TableRow>
                      ...
                      <TableHead>
                        <Button variant="ghost" size="sm" onClick={() => handleSort('price')}>
                          Reg. Price <ArrowUpDown className="w-3 h-3 ml-1" />
                        </Button>
                      </TableHead>
                      <TableHead>
                        <Button variant="ghost" size="sm" onClick={() => handleSort('salePrice')}>
                          Sale Price <ArrowUpDown className="w-3 h-3 ml-1" />
                        </Button>
                      </TableHead>
                  ...
                  <TableBody>
                    {filteredDomains.map((domain, index) => (
                      <TableRow key={domain.id}>
                        ...
                        <TableCell>
                          {domain.salePrice ? (
                            <span className="font-bold text-primary">${domain.salePrice}</span>
                          ) : (
                        ...
                        <TableCell className="text-right">
                          <div className="flex items-center justify-end gap-2">
                            <Button
                              ...
                              className={isFavorite(domain.name) ? 'text-red-500' : ''}
                            >
                    ...
                    <CardContent className="p-4">
                      <div className="flex justify-between items-start mb-3">
                        ...
                        <Button
                          ...
                          size="icon"
                          onClick={() => toggleFavorite(domain.name)}
                          className={`h-8 w-8 ${isFavorite(domain.name) ? 'text-red-500' : ''}`}
                        >
                          <Heart className={`w-4 h-4 ${isFavorite(domain.name) ? 'fill-current' : ''}`} />
                        </Button>
                      </div>
                      ...
                      <div className="flex justify-between items-center mb-3">
                        ...
                        {domain.salePrice ? (
                          <span className="font-bold text-primary text-lg">${domain.salePrice}</span>
                        ) : (
